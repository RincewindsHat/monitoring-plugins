...
int process_arguments (int, char **, check_disk_config_t *config);
void print_path (const char *mypath);
void set_all_thresholds (struct parameter_list *path, check_disk_config_t *config);
...
void stat_path (struct parameter_list *p, check_disk_config_t *config);
void get_stats (struct parameter_list *p, struct fs_usage *fsp, check_disk_config_t *config);
void get_path_stats (struct parameter_list *p, struct fs_usage *fsp, check_disk_config_t *config);
...
main (int argc, char **argv)
{
  ...
  if (config.path_selected == false) {
    for (me = mount_list; me; me = me->me_next) {
      ...
      path->best_match = me;
      path->group = group;
      set_all_thresholds(path, &config);
    }
  }
  np_set_best_match(path_select_list, mount_list, config.exact_match);
  ...
  for (path = path_select_list; path; path=path->name_next) {
    ...
    if (path->group == NULL) {
      ...
      if (me->me_remote && config.show_local_fs_only) {
        if (config.stat_remote_fs)
          stat_path(path, &config);
        continue;
  ...
  printf ("DISK %s%s%s|%s\n", state_text (result), (config.erronly && result==STATE_OK) ? "" : preamble, output, perf);
  return result;
}
...
int
process_arguments (int argc, char **argv, check_disk_config_t *config)
{
  int c, err;
  ...
  while (1) {
    ...
    case 'w':                 /* warning threshold */
      if (strstr(optarg, "%")) {
        if (*optarg == '@') {
          config->warn_freespace_percent = optarg;
        } else {
          xasprintf(&config->warn_freespace_percent, "@%s", optarg);
        }
      } else {
        if (*optarg == '@') {
          config->warn_freespace_units = optarg;
        } else {
          xasprintf(&config->warn_freespace_units, "@%s", optarg);
        }
      }
      break;
    ...
    */
    case 'c':                 /* critical threshold */
      if (strstr(optarg, "%")) {
        if (*optarg == '@') {
          config->crit_freespace_percent= optarg;
        } else {
          xasprintf(&config->crit_freespace_percent, "@%s", optarg);
        }
      } else {
        if (*optarg == '@') {
          config->crit_freespace_units = optarg;
        } else {
          xasprintf(&config->crit_freespace_units, "@%s", optarg);
        }
      }
      break;
    ...
    case 'W':            /* warning inode threshold */
      if (*optarg == '@') {
        config->warn_freeinodes_percent = optarg;
      } else {
        xasprintf(&config->warn_freeinodes_percent, "@%s", optarg);
      }
      break;
    case 'K':            /* critical inode threshold */
      if (*optarg == '@') {
        config->crit_freeinodes_percent = optarg;
      } else {
        xasprintf(&config->crit_freeinodes_percent, "@%s", optarg);
      }
      break;
    case 'u':
      if (config->units)
    ...
    case 'L':
      ...
      /* fallthrough */
    case 'l':
      config->show_local_fs_only = true;
      break;
    case 'P':
      config->display_inodes_perfdata = true;
      break;
    case 'p':                 /* select path */
      if (! (config->warn_freespace_units || config->crit_freespace_units || config->warn_freespace_percent ||
    ...
    case 'C':
      ...
      config->warn_freespace_units = NULL;
      config->crit_freespace_units = NULL;
      config->warn_usedspace_units = NULL;
      config->crit_usedspace_units = NULL;
      config->warn_freespace_percent = NULL;
      config->crit_freespace_percent = NULL;
      config->warn_usedspace_percent = NULL;
      config->crit_usedspace_percent = NULL;
      config->warn_usedinodes_percent = NULL;
      config->crit_usedinodes_percent = NULL;
      config->warn_freeinodes_percent = NULL;
      config->crit_freeinodes_percent = NULL;
  ...
  c = optind;
  if (config->warn_usedspace_percent == NULL && argc > c && is_intnonneg (argv[c]))
    config->warn_usedspace_percent = argv[c++];

  if (config->crit_usedspace_percent == NULL && argc > c && is_intnonneg (argv[c]))
    config->crit_usedspace_percent = argv[c++];
...
void
set_all_thresholds (struct parameter_list *path, check_disk_config_t *config)
{
    if (path->freespace_units != NULL) free(path->freespace_units);
    set_thresholds(&path->freespace_units, config->warn_freespace_units, config->crit_freespace_units);
    if (path->freespace_percent != NULL) free (path->freespace_percent);
    set_thresholds(&path->freespace_percent, config->warn_freespace_percent, config->crit_freespace_percent);
    if (path->usedspace_units != NULL) free (path->usedspace_units);
    set_thresholds(&path->usedspace_units, config->warn_usedspace_units, config->crit_usedspace_units);
    if (path->usedspace_percent != NULL) free (path->usedspace_percent);
    set_thresholds(&path->usedspace_percent, config->warn_usedspace_percent, config->crit_usedspace_percent);
    if (path->usedinodes_percent != NULL) free (path->usedinodes_percent);
    set_thresholds(&path->usedinodes_percent, config->warn_usedinodes_percent, config->crit_usedinodes_percent);
    if (path->freeinodes_percent != NULL) free (path->freeinodes_percent);
    set_thresholds(&path->freeinodes_percent, config->warn_freeinodes_percent, config->crit_freeinodes_percent);
}
...
print_help (void)
{
  ...
  printf (" %s\n", "-u, --units=STRING");
  printf ("    %s\n", _("Choose bytes, kB, MB, GB, TB (default: MiB)"));
  printf (" %s\n", "-m, --megabytes");
  printf ("    %s\n", _("Same as '--units MB'"));
  printf (" %s\n", "-k, --kilobytes");
  printf ("    %s\n", _("Same as '--units kB'"));
...
void
stat_path (struct parameter_list *p, check_disk_config_t *config)
{
...
void
get_stats (struct parameter_list *p, struct fs_usage *fsp, check_disk_config_t *config) {
...
void
get_path_stats (struct parameter_list *p, struct fs_usage *fsp, check_disk_config_t *config) {
